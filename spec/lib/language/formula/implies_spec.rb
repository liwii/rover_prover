...
describe Implies do
  ...
  let (:n_p) { Not.new(p) }
  let (:i1) { Implies.new(p, q) }
  let (:i2) { Implies.new(p, n_p) }

  describe 'free_variables' do
    it 'returns an array with only var' do
      expect(i2.free_variables).to match_array [x_var]
    end
  end

  describe 'free_unification_terms' do
    it 'returns an array with only uni' do
      expect(i2.free_unification_terms).to match_array [y_uni]
    end
  end

  describe 'replace' do
    it 'returns new if argument is equal to itself' do
      expect(i1.replace(i1, i2)).to eql i2
    end

    it 'returns itself if argument is not equal to itself' do
      expect(i1.replace(i2, i2)).to eql i1
    end

    it 'replace terms in the expression' do
      expect(i1.replace(q, n_p)).to eql i2
    end
  end

  describe 'occurs' do
    it "returns true if its terms contain the argument" do
      expect(i1.occurs(y_uni)).to be true
    end
    it "returns false if none of its terms is equal to the argument" do
      expect(i1.occurs(z_var)).to be false
    end
  end

  describe 'set_instantiation_time' do
    let (:new_time) { 10 }
    before { i1.set_instantiation_time(new_time) }
    it 'updates time' do
      expect(i1.formula_a.terms.all? { |x| x.time == new_time }).to be true
      expect(i1.formula_b.terms.all? { |x| x.time == new_time }).to be true
    end
  end
  ...
  describe 'eql?' do
    ...
    it "returns false if its terms aredifferent from the argument's" do
      expect(i1).not_to eql i2
    end

    it "returns false if the argument is not 'Implies'" do
      expect(i1).not_to eql f
    end
  end
end
